...
function loadEntries() {
  try {
    ...
    return raw ? JSON.parse(raw) : []; // json -> 객체로 변환 없으면 빈 배열 반환
  } catch {
    return [];
    // json 깨졌을 때 앱 터지는 거 방지
  }
}
...
function App() {
  // 처음 실행 시 loadEntries() 실행 -> 기존 데이터가 있으면 불러옴
  const [entries, setEntries] = useState(loadEntries);
  useEffect(() => {
    saveEntries(entries); // 자동으로 loadEntries에 저장됨
