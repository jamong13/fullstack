...
export default function WritePage() {
  ...
  useEffect(() => {
    // AuthContext가 member를 불러오는 동안 loading이 true일 수 있음
    if (!loading && !member) {
      // 로딩이 끝났는데 Member가 없으면 비로그인 -> login으로 보냄
  ...
  useEffect(() => {
    api.getCategories().then((cats) => {
      ...
      setCategories(cats); // 받은 목록 categories 설정
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      navigate(`/posts/${id}`); // 성공하면 -> 방금 쓴 글 상세로 이동
    } catch (err) {
      // 실패 시 에러
